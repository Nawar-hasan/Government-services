...
def forbidden(e):
    ...
    # بمجرد التحقق من الدفع، قم بتحديث حالة الطلب
    order.is_done = True
    db.session.commit()

    return 'Payment verified and order activated!'
...
def page_not_found(e):
    ...
    # إنشاء طلب
    order = Order(
        details='Service request details here...',
        service_id=service_id,
        user_id='user_id',  # يجب استبداله بمعرف المستخدم
        is_done=False
    )
    ...
    if payment_method == 'bank_transfer' and payment_receipt:
        # حفظ صورة الإشعار
        receipt_path = os.path.join(app.config['UPLOAD_DIRECTORY'], payment_receipt.filename)
        payment_receipt.save(receipt_path)
        # يمكنك إضافة منطق التحقق من الإشعار هنا

    return redirect(url_for('request_status', order_id=order.id))
    ...
    order = Order.query.get(order_id)  # احصل على الطلب بناءً على order_id

    # إنشاء PDF
    html_content = render_template('pdf_template.html', order=order)
    pdf_file = HTML(string=html_content).write_pdf()

    # إرسال البريد الإلكتروني
    msg = Message('Your Service Request PDF', recipients=[order.user.email])
    msg.body = 'Please find attached the PDF for your service request.'
    msg.attach('service_request.pdf', 'application/pdf', pdf_file)
    
    with app.app_context():
        mail.send(msg)
    
    return 'PDF sent to customer successfully!'

app.config['MAIL_SERVER'] = 'smtp.example.com'  # استبدل بمزود خدمة البريد الإلكتروني الخاص بك
